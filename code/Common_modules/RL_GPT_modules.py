...
class BaseGPTWrapper():
    ...
    def unroll_target(self, tgt, custom_mask=None, init_mem=None, reward1=0, reward2=0, is_reinforce = False, gamma = 0):
        ...
        probs, log_probs, _ = self.prob_map(inp_x, init_mem)
...
#             discounted_reward1_mean = np.mean(discounted_reward1, axis = 1)[:,np.newaxis]
#             discounted_reward1_std = np.std(discounted_reward1, axis = 1)[:,np.newaxis]
#             normalized_reward1 = (discounted_reward1 - discounted_reward1_mean) / (discounted_reward1_std + 1e-8)
            
            discounted_reward2 = [[score*(gamma**(idx)) for idx in range(log_probs.shape[1])] for score in reward2]
#             discounted_reward2_mean = np.mean(discounted_reward2, axis = 1)[:,np.newaxis]
#             discounted_reward2_std = np.std(discounted_reward2, axis = 1)[:,np.newaxis]
#             normalized_reward2 = (discounted_reward2 - discounted_reward2_mean) / (discounted_reward2_std + 1e-8)
            
            log_likes1 = (log_probs*one_hot_labels).sum(-1)*torch.tensor(discounted_reward1).cuda()
            log_likes2 = (log_probs*one_hot_labels).sum(-1)*torch.tensor(discounted_reward2).cuda()
#             log_likes3 = (log_probs*one_hot_labels).sum(-1)*torch.tensor(discounted_reward3).cuda()
...
#             masked_log_likes3 = non_pad_where * log_likes3  # make <PAD> positions zero
#             NLLLoss3 = -masked_log_likes3.sum(-1)
...
class GPTGenerator():
    ...
    def train_n_epochs(self, dataset:StringDataset, epochs:int, save_period=1, debug=None):
        """ make sure you called train_mode() first, or manually setting which component to train """
        dldr = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, collate_fn=dataset.collate_fn)
        epo_loss_list = []
        for epo in range(1, epochs+1):
            ...
            ppl = compute_perplexity(np.log10(likelihoods.cpu().detach()), np.array([self.base_gpt.max_len]*len(likelihoods.cpu().detach())))
            print("-- PPL mean : ", ppl)

            self.prog_num += 1
            self.lr_schedule.check_and_step()
            if self.prog_num % save_period == 0:
                self.save_ckpt_path()
    ...
    def save_ckpt_path(self):
        saveto = self.config.ckpt_path.format(str(self.prog_num))
